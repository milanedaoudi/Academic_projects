...
python soft/init2.py

# Verify files were created
if [ ! -f "Pin1_16bit.txt" ] || [ ! -f "Pin2_16bit.txt" ] || [ ! -f "results_float_16bit.txt" ]; then
    echo "ERROR: Python script failed to generate required files"
    exit 1
fi
...
vlog -work work posit_add_16bit_tb.v posit_add.v
...
vsim -c -do "run 2ms; quit" work.posit_add_16bit_tb_v

# Verify simulation output
if [ ! -f "posit_results_16bit.txt" ]; then
...
echo "- Pin1_16bit.txt    : Input test vectors 1"
echo "- Pin2_16bit.txt    : Input test vectors 2"
echo "- results_float_16bit.txt: Reference float results"
echo "- posit_results_16bit.txt: Verilog posit outputs"
echo "- error_16bit.txt        : Calculated errors"
echo ""
