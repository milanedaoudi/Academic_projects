...
def calculate_errors():
    ...
    with open("error.txt", "w") as f_error, \
         open("large_errors.log", "w") as f_large:
        
        f_large.write("Lines with absolute error > 100:\n")
        total_error = 0.0
        valid_count = 0
        large_error_count = 0
        ...
        for i in xrange(len(posit_lines)):
            try:
                ...
                # Handle large errors
                if error > 10:
                    f_large.write("Line {}: Posit={}, Float={}, Error={:.15g}\n".format(
    ...
    if valid_count > 0:
        print("Average error (excluding >100): {:.15g}".format(total_error/valid_count))
    else:
        print("No valid error calculations could be performed")
    
    print("\nResults saved to:")
    print("- error.txt")
    print("- large_errors.log")
