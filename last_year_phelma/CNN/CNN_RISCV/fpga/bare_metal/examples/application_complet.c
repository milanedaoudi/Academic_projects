              ...
              uint8_t *global_tab)
{
  ...
  int idx = n_image - 1;   // Convention claire

  uint8_t *pixels = malloc(DISPLAY_IMAGE_SIZE * 3);
  ...
  printf("[read_pic] Image %d : %dx%d\n", n_image, width, height);

  /* Read max color value (usually 255) */
  f_gets(line, 10000, &fil);

  int i = 0;
  while (i < size * 3 && f_gets(line, 10000, &fil))
  {
    tok = My_strtok(line, " ");
    while (tok && i < size * 3)
    {
  ...
  /* Copy into global_tab */
  for (i = 0; i < size * 3; ++i)
  {
    global_tab[idx * size * 3 + i] = pixels[i];
  }
...
void bouton(void)
{
  ...
  claim = plic[PLIC_HART0_CLAIM_COMPLETE_ADDR/4]; 									//consulter le fichier syscall.c
  clear_csr(mie, MIP_MEIP);
  if(isBouncing == 0)
  {
    // printf("Interrupt executed !\n");
  	// If BTNW :	//Si pression du bouton Ouest, décrémentation de la variable de sélection de l'image
  	if (claim == ID_BTNW)			//Mise à sa valeur max si elle atteint sa valeur min
  	{
  		imageSel-- ;
  		if(imageSel < 0)
			imageSel = NB_IMAGES_TO_BE_READ - 1; ;
    }
  	// If BTNE :	//Si pression du bouton Est, incrémentation de la variable de sélection de l'image
  	else if (claim == ID_BTNE)	//Mise à sa valeur min si elle atteint sa valeur max
  	{
      		imageSel++; ;
      if(imageSel >= NB_IMAGES_TO_BE_READ)
        	imageSel = 0;
  	}
  	// If BTNS ://Si pression du bouton Sud, décrémentation de la variable de sélection du filtre
  	else if (claim == ID_BTNS)	//Mise à sa valeur max si elle atteint sa valeur min
  	{
  		filterSel--;
  		if(filterSel<0){
		filterSel = CNN_CLASSIFIER;}
  	}
  	// If BTNN :	//Si pression du bouton Nord, incrémentation de la variable de sélection du filtre
  	else if (claim == ID_BTNN)//Mise à sa valeur min si elle atteint sa valeur max
  	{
      		filterSel++;
...
int main(void)
{
   ...
   for (n_image = MIN_IMAGES_TO_READ; n_image <= MAX_IMAGES_TO_READ; n_image++)
  {
    read_pic(n_image,
             ...
             tab_length,
             global_tab);
  }				// Lire chaque image et les stocker dans global_tab
  ...
  for (n_image = 1; n_image <= NB_IMAGES_TO_BE_READ; n_image++)
  {
    convert_to_greyscale(
        ...
        global_tab,
        TAB_GS[n_image - 1]);
  }				//Pour chaque image de global_tab, appliquer le greyscale et stocker le résultat dans TAB_GS 
  // FILTERING STUFF
  printf("Starting filtering!\n");
   for (n_image = 0; n_image < NB_IMAGES_TO_BE_READ; n_image++)
  {
    convolution_filter(
        TAB_GS[n_image],
        kernel,
        biaises,
        TAB_GS_FILTERED[n_image]);
  }                          //Pour chaque image de TAB_GS, appliquer la convolution et les stocker dans TAB_GS_FILTERED
  ...
  while (1)
  {
    if ((imageSel != previous_imageSel) ||
        (filterSel != previous_filterSel))    //Comparaison des valeurs courantes et précédentes des variables de sélection de l'image et du filtre
    {
      if (filterSel != previous_filterSel)
      {
        edgeDetectorDone = 0;
        CNNDone = 0;
      }
      ...
      display(imageSel + 1,
              filterSel,
              previous_imageSel,
              previous_filterSel);//Si différence, maise à jour de l'affichage
