...
//------------------ORDAN-START--------------------
void display(int img_in_number, filter_type filter_nb, int previous_imageSel, int previous_filterSel) //, uint8_t *edgeDetectorDone, uint8_t *CNNDone)
{
  ...
  ptr_selected_img_filtered = (uint8_t *)(TAB_GS_FILTERED[img_in_number - 1]);
...
int main(void)
{
    printf("\n===== IMAGE VIEWER + FILTER NAVIGATION (ISR) =====\n");

    init_csrs(); // Initialisation CSRs
    ...
    printf("[MAIN] Loading images from SD card...\n");

    int tab_size[NB_IMAGES_TO_BE_READ]   = {0};
    int tab_width[NB_IMAGES_TO_BE_READ]  = {0};
    int tab_length[NB_IMAGES_TO_BE_READ] = {0};

    for (int n_image = MIN_IMAGES_TO_READ; n_image <= MAX_IMAGES_TO_READ; n_image++)
    {
        read_pic(n_image, tab_size, tab_width, tab_length, global_tab);
        printf("[MAIN] Image %d loaded: %dx%d (%d pixels)\n",
               n_image,
               tab_width[n_image-1],
    ...
    // SD unmounted (on n'a plus besoin du RAW)
    f_mount(NULL, "", 1);

    // Convertir toutes les images en niveaux de gris
    for (int n_image = 1; n_image <= NB_IMAGES_TO_BE_READ; n_image++)
    {
    ...
    // Appliquer la convolution sur toutes les images
    for (int n_image = 0; n_image < NB_IMAGES_TO_BE_READ; n_image++)
    {
        convolution_filter(
        TAB_GS[n_image],
        kernel,
    ...
    printf("[MAIN] Initialization done, starting VGA loop...\n");

    // Activer interruptions externes pour boutons
    enable_plic_interrupts();

    uint8_t previous_imageSel = 255;
    uint8_t previous_filterSel = 255;

    // Boucle principale d'affichage
    while (1)
    {
        // Mise à jour affichage si image ou filtre changé
        if ((imageSel != previous_imageSel) || (filterSel != previous_filterSel))
        {
            display(imageSel + 1, filterSel, previous_imageSel, previous_filterSel);

            previous_imageSel = imageSel;
            previous_filterSel = filterSel;
        }

        // Simple delay pour boucle (ISR gère boutons)
        for (volatile unsigned int ii = 0; ii < 10000; ii++);
    }
